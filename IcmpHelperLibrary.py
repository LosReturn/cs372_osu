...
# Due Date: May 1, 2022
# Date Modified: April 30, 2022
# Description: Working with ICMP Traceroute

# Code Adapted From: 
# https://github.com/avaiyang/ICMP-Traceroute
...
class IcmpHelperLibrary:
    ...
    class IcmpPacket:
        ...
        def __validateIcmpReplyPacketWithOriginalPingData(self, icmpReplyPacket):
            ...
            if self.getPacketSequenceNumber() != icmpReplyPacket.getIcmpSequenceNumber():
                print("The Sent Sequence Number: ", self.getPacketSequenceNumber(), "Received!", icmpReplyPacket.getIcmpSequenceNumber())
            if self.getPacketIdentifier() != icmpReplyPacket.getIcmpIdentifier():
                print("The Sent Packet Identifier: ", self.getPacketIdentifier(), "Received!", icmpReplyPacket.getIcmpIdentifier())
            if self.getDataRaw() != icmpReplyPacket.getIcmpData():
                print("The Sent Data: ", self.getDataRaw(), "Received", icmpReplyPacket.getIcmpData())
            if self.getPacketSequenceNumber() == icmpReplyPacket.getIcmpSequenceNumber() and self.getPacketIdentifier() == icmpReplyPacket.getIcmpIdentifier() and self.getDataRaw() == icmpReplyPacket.getIcmpData():
                ...
                droppedPacket += 1
                print("Dropped Packet Detected!", droppedPacket)
    ...
    class IcmpPacket_EchoReply:
        ...
        def printResultToConsole(self, ttl, timeReceived, addr):
            if(self.isValidResponse() == True):
                ...
                if ((self.getIcmpType() == 0) and (self.getIcmpCode() == 0)):
                    iccode.append(self.getIcmpCode())
                    ictype.append(self.getIcmpType())
                
                RTTTimes.append((timeReceived - TimeSent) * 1000)

            else:
                if self.getIsValidSequenceNumber() == False:
                    print("The Expected Sequence Number: ", expectedSequenceNumber, "Received!", self.getIcmpSequenceNumber())
                if self.getIsValidPacketIdentifier() == False:
                    print("The Expected Packet Identifier: ", expectedIdentifier, "Received!", self.getIcmpIdentifier())
                if self.getIsValidRawData() == False:
                    print("The Expectred Raw Data: ", expectedData, "Received!", self.getIcmpData())
    ...
    def __sendIcmpEchoRequest(self, host):
        ...
        for i in range(4):
            ...
            timeReceived = time.time()
            counter += 1
        totalRTT += (timeReceived - TimeSent) * 1000
        averageRTT = totalRTT / counter
        print("Lowest RTT: %.0f ms" %min(RTTTimes))
        print("Highest RTT: %.0f ms" %max(RTTTimes))
        print("Average RTT: %.0f ms" %averageRTT)
        print("Percentage of Packet Lost: 0.0%")
        

    def __sendIcmpTraceRoute(self, host):
        print("sendIcmpTraceRoute Started...") if self.__DEBUG_IcmpHelperLibrary else 0
        # Build code for trace route here
        print("Over Maximum of 30 Hops!")
        print("Where the TTL is the Hop Number")
        for i in range(30):
            icmpPacket = IcmpHelperLibrary.IcmpPacket()
            ...
            if len(iccode) > 0 and len(ictype) > 0:
                if iccode[0] == 0 and ictype[0] == 0:
                    break;
